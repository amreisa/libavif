...
int avif_encoding( char **outputFile, const char *inputFile, int argc, char **argv ) {
    int funVal = 0;

    printf( "App with full path name %s\n", argv[0] );
 ...
 /////
    ...
    ///////////////////////////////////
    // Copy data that from raw.data to ----->> outputFile function reference.
////    *outputFile = raw.data;

//    // for Debug only
//    size_t outputFileSize = sizeof( outputFileName );
//    printf( "The size for Raw Data = %zu. And the size for outputfile = %zu.\n", raw.size, outputFileSize );

//    //
//    FILE * f = fopen( outputFileName, "wb" );

//    // For debug only.
//    printf( "The size for output file data = %zu.\n", strlen( outputFileName ) );

//    if (!f) {
//        // change that now not have file name it have file data, so I mast have file name
...
//    fclose(f);
    ...
    *outputFile = malloc( raw.size );
    ...
    fileDataSize = raw.size;

    memcpy( *outputFile, raw.data, raw.size );
...
// TODO : Remove me.
avifBool fileNameHaveExt( const char *fileName, const char *fileExt ) {
    // FIXME : The fileExt not at the end off the string
...
{
    // TODO : Add arg to handel dir so we can sacn all file that can be encodeing.
    // TODO : make loop for that files.
    DIR *d;
    struct dirent *dir;
    d = opendir( "." );
    if ( d ) {
        while ( ( dir = readdir( d ) ) != NULL ) {
            if ( strcmp( dir->d_name, "." )!=0 && strcmp( dir->d_name, ".." ) != 0 ) {
                //                printf( "%s\n", dir->d_name );
                //                printf( "The file have Ext %d.\n", fileNameHaveExt( dir->d_name, ".jpg" ) );
    ...
    for ( int argcNumber = 1 ; argcNumber < argc ; ++argcNumber ) {
        if ( strstr( argv[ argcNumber ], "--" ) || strstr( argv[ argcNumber ], "-" ) ) {
            ...
            if ( !strcmp( argv[ argcNumber ], "-o" ) || !strcmp( argv[ argcNumber ], "--output" ) ) {
                // Debug message.
                printf( "Output file name will be \"%s\".\n", argv[ argcNumber+1 ] );
                //
                outputFileName = argv[ argcNumber + 1 ];
            } else {
            }
        } else {
            printf( "The file that will be encodeing\"%s\".\n", argv[ argcNumber ] );
        }
    }

    int returnCodeFromFun = avif_encoding( &outputFileData, inputFileData, argc, argv );
    // For debug only.
    printf( "The size for output file data = %zu.\n", fileDataSize );

    if ( fileDataSize != 0 || returnCodeFromFun == 0 ) {
        //outputFileData = malloc( fileDataSize );

//        int returnCodeFromFun = avif_encoding( &outputFileData, inputFileData, argc, argv );

        if ( returnCodeFromFun == 0 ) {
            FILE * f = fopen( outputFileName, "wb" );
            ...
            if ( fwrite( outputFileData, 1, fileDataSize, f ) != fileDataSize ) {
                fprintf( stderr, "Failed to write %" AVIF_FMT_ZU " bytes: %s\n", fileDataSize, outputFileName );
                returnCode = 1;
            } else {
                printf( "Wrote AVIF: %s\n", outputFileName );
            }
            ...
            fprintf( stderr, "ERROR: Failed to encoding file.\n" );
            returnCode = 1;
            goto cleanup;
        }
    } else {
        fprintf( stderr, "ERROR: Failed to the get file size.\n" );
        returnCode = 1;
        goto cleanup;
    }
...
cleanup:
    ...
    free( ( char* )outputFileData );
    free( ( char* )outputFileName );
    free( ( char* )inputFileData );

    printf("The App will end now.\n");
